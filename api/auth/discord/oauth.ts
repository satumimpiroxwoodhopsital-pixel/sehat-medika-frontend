...
export default async function handler(req: Request) {
  try {
    ...
    if (!clientId || !clientSecret || !redirectUri) {
      return new Response('Missing environment variables. Check Vercel env vars: VITE_DISCORD_CLIENT_ID, DISCORD_CLIENT_SECRET, DISCORD_REDIRECT_URI', { status: 500 });
    }
    ...
    if (!discordUser.id) {
      return new Response('Failed to get Discord user info: ' + JSON.stringify(discordUser), { status: 400 });
    }
    ...
    // Step5: Find admin user in Supabase
    ...
    return new Response('Server error: ' + (error?.message || String(error)), { status: 500 });
  }
}
